...
public:
   ...
   /**
    * set the color, animation mode, and direction of a sprite
    * @param color currently only 2-bit set colors
    * @param auto_sel sets animation mode, 0 = static, 1 = auto
    * @param direction based on sprite sheet
    *
    * @note COLOR, ANIMATION MODE, AND DIRECTION MUST BE SET AT THE SAME TIME
    *
    */
   void set_color_anim_dir(int32_t color, int auto_sel, int direction);

   /**
       * clears sprite by moving it off-screen
       * @param sprite to move off-screen
       *
       * @note assuming 640x480 size
       */
   void clr_sprite();
...
public:
   ...
   enum {
      BYPASS_REG = 0x2000,     /**< bypass control register */
	  X_REG_START = 0x2001,  /**< start of x-coordinate registers */
	  Y_REG_START = 0x2002,  /**< start of y-coordinate registers */
	  CTRL_REG_START = 0x2003 	/**< start of command registers */
   };
   /**
    * symbolic constants
    *
   ...
   /* methods */
   DupliCore(uint32_t core_base_addr, int size);
   ~DupliCore();                  // not used
    ...
    * @param cmd control command
    *
    * @note 1-bit sprite select _ 2-bit color select
    */
   void wr_ctrl(int32_t cmd, int sprite);

   /**
    * set the color and direction of a sprite
    * @param color currently only 2-bit set colors
    * @param direction based on sprite sheet
    * @param sprite to modify
    *
    * @note COLOR AND DIRECTION MUST BE SET AT THE SAME TIME
    *
    */
   void set_color_dir(int32_t color, int direction, int sprite);
    ...
    * @note origin is the top-left corner of sprite
    */
   void move_xy(uint32_t x, uint32_t y, int sprite);
  ...
  /**
      ...
      * @note assuming 640x480 size
      */
 void clr_sprite(int sprite);

 /**
     * clears all sprites by moving them off-screen
     *
     * @note assuming 640x480 size
     */
void clr_all();
